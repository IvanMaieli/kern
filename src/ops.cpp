...
namespace kern::ops {
    ...
    void Add(const Tensor& a, const Tensor& b, Tensor& out) {
        ...
        if (a.dtype() == DataType::float32 && a.shape() == b.shape() && a.shape() == out.shape() &&
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                std::array<Shape::Dimension, Shape::maximum_rank> a_coords{}, b_coords{};
                            ...
                            for (size_t n_tile = 0; n_tile < N; n_tile += TILE_SIZE) {
                                for (size_t m = m_tile; m < std::min(m_tile + TILE_SIZE, m_end); ++m) {
                                    for (size_t k = k_tile; k < std::min(k_tile + TILE_SIZE, K); ++k) {
                                        ...
                                        for (; n + 7 < n_end; n += 8) {
                                            float32x4_t vb1 = vld1q_f32(b_ptr + b.shape().linear_index({k, n}));
                                            ...
                                            float32x4_t vb2 = vld1q_f32(b_ptr + b.shape().linear_index({k, n + 4}));
                                        ...
                                        for (; n < n_end; ++n) out_ptr[out.shape().linear_index({m, n})] += a_val * b_ptr[b.shape().linear_index({k, n})];
                                    }
                                }
                            }
                        }
    ...
    void MatMulTransposed(const Tensor& a, const Tensor& b_t, Tensor& out) {
        ...
        if (a.dtype() == DataType::float32) {
            ...
            for (unsigned int t = 0; t < num_threads; ++t) {
                ...
                size_t m_end = std::min(m_start + rows_per_thread, M);
                if (m_start >= m_end) break;
                threads.emplace_back([=, &a, &b_t, &out]() {
                    for (size_t m = m_start; m < m_end; ++m) {
                        for (size_t n = 0; n < N; ++n) {
                            float sum = 0.0f;
                            size_t k = 0;
                            for (; k + 3 < K; k += 4) {
                                float32x4_t va = vld1q_f32(a_ptr + a.shape().linear_index({m, k}));
                                float32x4_t vb = vld1q_f32(b_ptr + b_t.shape().linear_index({n, k}));
                                float32x4_t vprod = vmulq_f32(va, vb);
                                sum += vgetq_lane_f32(vprod, 0) + vgetq_lane_f32(vprod, 1) + 
                                       vgetq_lane_f32(vprod, 2) + vgetq_lane_f32(vprod, 3);
                            }
                            for (; k < K; ++k) sum += a_ptr[a.shape().linear_index({m, k})] * b_ptr[b_t.shape().linear_index({n, k})];
    ...
    void ReLU(const Tensor& in, Tensor& out) {
        ...
        if (in.dtype() == DataType::float32) {
            ...
            for (size_t i = 0; i < n; ++i) out_ptr[i] = std::max(0.0f, in_ptr[i]);
